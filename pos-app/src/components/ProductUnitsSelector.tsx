...
export default function ProductUnitsSelector({ productId, onClose }: ProductUnitsSelectorProps) {
  ...
  const { data: unitsData, isLoading, error } = useQuery({
    ...
    queryFn: () => fetchAvailableUnits(productId),
  });

  // Mutations
  const assignMutation = useMutation({
    mutationFn: ({ unitId, factor }: { unitId: number; factor: number }) =>
      assignUnit(productId, unitId, factor),
    onSuccess: () => {
  ...
  const removeMutation = useMutation({
    mutationFn: (unitId: number) => removeUnit(productId, unitId),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['productUnits', productId] });
    },
  });
  ...
  const updateFactorMutation = useMutation({
    mutationFn: ({ unitId, factor }: { unitId: number; factor: number }) =>
      updateFactor(productId, unitId, factor),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['productUnits', productId] });
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg p-6 max-w-4xl w-full mx-4 max-h-[80vh] overflow-y-auto">
        ...
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-xl font-semibold text-gray-900">Gestionar Unidades del Producto</h2>
            <p className="text-gray-600">Asigna unidades de medida disponibles para este producto</p>
          </div>
          <button
            onClick={onClose}
